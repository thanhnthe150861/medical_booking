...
public class PatientDBContext extends  DBContext{
    ...
    public List<Doctor> checkDateSlotEmpty(String date, String slot){// Kiểm tra xem có bác sĩ nào có lịch đặt vào ngày và slot đó chưa
        List<Doctor> doctorList = new ArrayList<>();
        try {
            String sql = "SELECT \n" +
                    "    booking.id,\n" +
                    ...
                    "LEFT JOIN rank_doctor ON doctor.rank_id = rank_doctor.id\n" +
                    "LEFT JOIN slot ON booking.slot_id = slot.id\n" +
                    "WHERE booking.status != 'Cancelled' and booking.date = ? and slot_id = ?;;";
            stm = connection.prepareStatement(sql);
            stm.setDate(1, Date.valueOf(date));
            stm.setInt(2, Integer.parseInt(slot));
            rs = stm.executeQuery();
            while (rs.next()){
                Doctor doctor = new Doctor();
                doctor.setId(rs.getInt("doctor_id"));
